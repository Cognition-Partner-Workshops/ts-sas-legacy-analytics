...
| `[FACT]` P1 banking / `daily_transaction_processing` | DONE | DONE | DONE | W2 B2 DONE (PR pending review; `databricks/src/jobs/daily_transaction_processing.py`, DEC-015 (a) literal: 10-col `daily_transactions`/`txn_rejected` (AMB-02/03), pre-append stats (AMB-13), missing-low anomaly order (AMB-06); idempotent: seed-when-empty + insert-only MERGE, date-slice REPLACE WHERE, `txn_rejected` wholesale overwrite (DEC-016); ad hoc `--executor warehouse` runs x2 — bundle run hit the shared `.bundle/sas_legacy/dev` path race with concurrent W2 children) | live PASS 2024-01-31 (mode DEGRADED; 64 PASS / 0 FAIL / 8 N/A / T-9 `txn_rejected` DECLARED-UNEXERCISED; 1 live run of cap 3; `databricks/evidence/w2_b2/`) | NOT STARTED |
| `[FACT]` P1 banking / `credit_risk_scoring` | DONE | DONE | DONE | W2 B3 DONE (PR pending; `databricks/src/jobs/credit_risk_scoring.py`, scorecard `CRM-2023-Q4-v2` transcribed literally, DEC-015 (a); idempotent per `score_date`; job run `730586794422400` task SUCCESS) | live PASS 2024-01-31 (mode DEGRADED; 96 PASS / 0 FAIL / 10 N-A-or-INFO / 0 DECL-UNEX; ML-8 debug not materialised; `databricks/evidence/w2_b3/`) | NOT STARTED |
| `[FACT]` P1 banking / `monthly_regulatory_reporting` | DONE | DONE | DONE | W2 B4 DONE (PR pending review; `databricks/src/jobs/monthly_regulatory_reporting.py`, Spark SQL, literal DEC-015 (a)/AMB-07 `risk_weight` key; idempotent per `report_month` via `INSERT ... REPLACE WHERE`; xlsx `REG_REPORT_202401.xlsx` 3 sheets; job run `885378395536755` SUCCESS) | live PASS 2024-01-31 (mode DEGRADED; 70 PASS / 0 FAIL / 7 N/A; AMB-07 missing-MTG-LTV and `Unknown` bucket declared-unexercised by seed; `databricks/evidence/w2_b4/`) | NOT STARTED |
